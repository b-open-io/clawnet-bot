#!/usr/bin/env python3
"""Install, check, update, or uninstall Johnny's generated Codex adapter."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if manifest.get("manager") != "clawnet-bot" or [a.get("generated_file") for a in manifest.get("agents", [])] != [AGENT_FILE]:
        print("error: invalid or missing ClawNet Bot agent manifest")
        return 1
    ...
    ownership = load_json(ownership_path, {"manager": "clawnet-bot", "agents": {}})
    if ownership.get("manager") != "clawnet-bot":
        print(f"error: invalid ClawNet Bot ownership metadata: {ownership_path}")
        return 1
    record = ownership.get("agents", {}).get(AGENT_FILE)

    if args.uninstall:
        if not record:
            print(f"unchanged: {AGENT_FILE} is not managed by ClawNet Bot")
            return 0
    ...
    print("Start a new Codex session before invoking clawnet_mechanic.")
    return 0
