#!/usr/bin/env python3
"""Focused tests for ClawNet Bot Codex agent generation and installation."""
...
class AdapterTests(unittest.TestCase):
    ...
    def test_generated_toml_parses_and_preserves_canonical_body(self) -> None:
        ...
        raw = (ROOT / "agents" / "clawnet-mechanic.md").read_text(encoding="utf-8")
        body = raw.split("\n---\n", 1)[1]
        self.assertEqual(parsed["name"], "clawnet_mechanic")
        self.assertTrue(parsed["developer_instructions"].endswith("\n" + body))
        self.assertIn("Codex compatibility prelude (ClawNet Bot)", parsed["developer_instructions"])
    ...
    def test_update_replaces_unmodified_managed_file(self) -> None:
        ...
        ownership_path = target_dir / ".clawnet-bot-agents.json"
        ownership = json.loads(ownership_path.read_text())
        old = 'name = "clawnet_mechanic"\n'
    ...
    def test_modified_managed_update_requires_force(self) -> None:
        self.assertEqual(self.run_installer().returncode, 0)
        target = self.base / ".codex" / "agents" / AGENT
        ...
        self.assertEqual(target.read_text(), "user-modified managed adapter\n")
    ...
    def test_unmanaged_collision_refused_and_force_quarantines(self) -> None:
        ...
        recovered = list((target_dir / ".clawnet-bot-agents-trash" / "quarantine").glob(f"{AGENT}*"))
    ...
    def test_uninstall_modified_managed_requires_force_and_quarantines(self) -> None:
        self.assertEqual(self.run_installer().returncode, 0)
        target_dir = self.base / ".codex" / "agents"
        target = target_dir / AGENT
        target.write_text("user-modified managed adapter\n")
    ...
    def test_foreign_ownership_metadata_is_refused(self) -> None:
        target_dir = self.base / "custom"
        target_dir.mkdir()
        (target_dir / ".clawnet-bot-agents.json").write_text('{"manager":"other","agents":{}}\n')
        result = self.run_installer("--target", str(target_dir))
        self.assertEqual(result.returncode, 1)
        self.assertIn("invalid ClawNet Bot ownership metadata", result.stdout)
        self.assertFalse((target_dir / AGENT).exists())
